<div>
    <button type="button" wire:click="openModal" class="px-3 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700">
        🔗 Добавить ссылку
    </button>
        ...
        <div style="position: fixed; inset: 0; z-index: 9999; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center;">
            <div style="background: white; border-radius: 0.5rem; width: 700px; max-width: 90%; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1);">
                ...
                <div style="padding: 1.5rem;">
                    ...
                    <!-- Вкладка Расширенные (заглушка) -->
                    @if($activeTab === 'advanced')
                        <div style="text-align: center; padding: 2rem; color: #6b7280;">
                            Расширенные настройки (будет позже)
                        </div>
                    @endif

                    <!-- Вкладка Всплывающие окна (заглушка) -->
                    @if($activeTab === 'popup')
                        <div style="text-align: center; padding: 2rem; color: #6b7280;">
                            Настройка всплывающих окон (будет позже)
                        </div>
                    @endif
    ...
    document.addEventListener('livewire:init', () => {
        Livewire.on('insert-link', (data) => {
            ...
            if (editor) {
                ...
                let linkText = data.text || selectedText || data.url;
                let linkHtml = `<a href="${data.url}" target="${data.target}" title="${data.title}">${linkText}</a>`;
